...
export class InvoiceComponent extends BaseComponent implements OnInit {
  ...
  printOnly() {
    if (this.validateForms()) {
      ...
      let modereplaced = modelreplaced.replace("{mode}", controls["mode"].value);

      const gstFormControls = this.getGSTFormControls;
      let b = modereplaced.replace("{sgst}", gstFormControls["sgst"].value);
      ...
      this.profileSerive
        ...
        .subscribe((result) => {
          ...
          const amountwithgst = ifsc.replace(
  ...
  saveOnly() {
    if (this.validateForms()) {
      if (this.gridDatas.length == 0) {
        this.updateToastMessage(
          "Atleast add one product to grid",
          Constants.error,
          "AdroMinds Invoice"
        );
        this.addItemFormsSubmitted = false;
